...
@synthesize delegate, headerView, contentView;

- (id)initWithOrigin:(CGPoint)origin delegate:(id<KalViewDelegate>)theDelegate logic:(KalLogic *)theLogic
{
    if ((self = [super initWithFrame:CGRectMake(origin.x, origin.y, 320.0f, 1)])) {
        ...
        contentView = [[[UIView alloc] initWithFrame:CGRectMake(0.f, kHeaderHeight, 320, 1)] autorelease];
...
- (void)showPreviousMonth
{
    if (!gridView.transitioning) {
        ...
        [self slideDown];
        if ([delegate respondsToSelector:@selector(showPreviousMonth)])
...
- (void)showFollowingMonth
{
    if (!gridView.transitioning) {
        ...
        [self slideUp];
        if ([delegate respondsToSelector:@selector(showFollowingMonth)])
...
- (void)addSubviewsToHeaderView:(UIView *)_headerView
{
    ...
    CGRect imageFrame = _headerView.frame;
    ...
    [_headerView addSubview:previousMonthButton];
    ...
    [_headerView addSubview:nextMonthButton];
    ...
    for (CGFloat xOffset = 0.f; xOffset < _headerView.width; xOffset += 46.f, i = (i+1)%7) {
...
- (void)addSubviewsToContentView:(UIView *)_contentView
{
    ...
    [gridView addObserver:self forKeyPath:@"frame" options:NSKeyValueObservingOptionNew|NSKeyValueObservingOptionOld context:NULL];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    if (object == gridView && [keyPath isEqualToString:@"frame"]) {
         ...
         */
        self.contentView.height = gridView.bottom;
        self.height = self.headerView.height + self.contentView.height;
